...
visit_num_filtered <- vitals_d %>%
  ...
  pull(visit_num) %>%
  unique()
length(visit_num_filtered)


sum(vitals_d$prbc_given == 1)
k_ext %>% 
  # filter((visit_num %in% visit_num_filtered)) %>%
  filter(gfr_mdrd_below_30) %>% 
  pull(visit_num) %>%
  # unique() %>% 
  length()

str(k_ext$performed_date)
table(k_ext$performed_unit_of_measure)
# Include patients:
k_filtered <- k_ext %>%
...
k_d %>% pull(visit_num) %>% unique() %>% length()
```
...
```{r}
k_ext %>% filter(on_tpn) %>% pull(visit_num) %>% unique() %>% length() # 8234
```
...
```{r}
k_ext %>% filter(gfr_mdrd_below_30) %>% pull(visit_num) %>% unique() %>% length() # 13675
