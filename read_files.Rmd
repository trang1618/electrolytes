...
system.time(
  k_ext <- k_d_all %>%
    mutate(delta_val = post_repletion_val - pre_repletion_val,
           performed_day_of_week = performed_date %>% ymd() %>% weekdays(),
...
k_filtered <- k_ext %>%
  ...
  filter(visit_num %in% visit_num_filtered & 
           !(on_tpn | # not on TPN
               is_rhabdo | # no rhabdomyolysis code
               is_parathyroid | # no parathyroid disorders of any kind
               is_sarcoid | # no sarcoid disorders of any kind
               is_esrd | # no code for ESRD
               is_dialysis | # no code for dialysis
               given_dialysis | # no dialysis procedure
               gfr_mdrd_below_30)
  ) %>%
